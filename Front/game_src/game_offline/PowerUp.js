...
export class PowerUp {
    constructor(scene, owner) {
		...
		this.isActive = false;
		this.originalSize = this.owner.paddleHeight;
		this.originalSpeed = this.owner.paddleSpeed;
	...
	getClanColor() {
        ...
        return clanColors[this.owner.clan];
    }

    spawn() {
        const maxX = (this.arena.arenaWidth / 2) - (this.size * 2);
        const maxY = (this.arena.arenaHeight / 2) - (this.size * 2);
        
        const randomX = (Math.random() * 2 - 1) * maxX;
        const randomY = (Math.random() * 2 - 1) * maxY;
        
		// this.mesh.position.set(0, 0, this.size + this.arena.arenaDepth / 2);
        this.mesh.position.set(randomX, randomY, this.initialZ);
